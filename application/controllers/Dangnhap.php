...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->data['title']='Fashion_Shop-Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $newcoupon=array(
                ...
                'description' => 'Nhập "GIAMGIA10K" để T.T.Anh giảm giá',
                'created' => $today,
                'orders' => 0,
                'trash' => 1,
                'status' => 1,
            ...
            $this->email->to($email);
            $this->email->subject('Fashion_Shop - tặng bạn voucher 10K');
            $this->email->message('Bạn đã trở thành thành viên của Fashion_Shop, tặng bạn 1 mã giảm giá giảm 10.000 đ : '.$tempcoupon.' , Hạn dùng'.$tempdatelimit.'
                Hãy sử dụng tài khoản để mua hàng để nhận nhiều ưu đãi');
            $this->email->send();
            $this->data['success']='Đăng ký thành công! Bạn đã nhận được 1 mã giảm giá 10k';

        }  
        $this->data['title']='Fashion_Shop - Đăng ký tài khoản';   
    ...
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['validation'] = TRUE;   
            $this->email->initialize($config);
            $this->email->from('sale.smart.store.2019@gmail.com', 'Fashion_Shop');
            $this->email->to($list['email']);
            $this->email->subject('Fashion_Shop - Lấy lại mật khẩu');
        ...
        $this->data['title']='Fashion_Shop - Quên mật khẩu';   
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='Fashion_Shop - Cập nhật mật khẩu mới';
